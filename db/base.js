...
module.exports = {
    ...
    insertarprd(nombre, precio, codigo, descripcion, empresa, sexo, categoria_id){
        return new Promise ((resolve, reject) =>{
            const sql = 'INSERT INTO productos (nombre, precio, codigo, descripcion, empresa, sexo, categoria_id) VALUES (?, ?, ?, ?, ?, ?, ?)';
            db.run (sql, [nombre, precio, codigo, descripcion, empresa, sexo, categoria_id], (err, resultados)=>{
                if (err) reject(err);
                else resolve(resultados);
            });
        });
    ...
    actualizarprd(nombre, precio, codigo, descripcion, empresa, sexo, id){
        return new Promise ((resolve, reject)=>{
            const sql = 'UPDATE productos SET nombre = ?, precio = ?, codigo = ?, descripcion = ?, empresa = ?, sexo = ? WHERE id = ?';
            db.run(sql, [nombre, precio, codigo, descripcion, empresa, sexo, id], (err)=>{
                if (err) reject(err);
                else resolve();
            });
        });
