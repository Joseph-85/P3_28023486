...
const {nombre,codigo,precio,descripcion,sexo,empresa,categoria} = req.body;
    
const sql = `INSERT INTO productos(nombre, codigo, precio, descripcion, sexo, empresa, categoria_id) 
    VALUES (?, ?, ?, ?, ?, ? , ?)`;

db.run(sql, [nombre, codigo, precio, descripcion, sexo, empresa, categoria], err => {
    if (err) return console.error(err.message);
...
function update(req,res){
  ...
  const sql = `UPDATE productos SET nombre = ?, codigo = ?, precio = ?, descripcion = ?, sexo = ?, empresa = ?, categoria_id = ? WHERE producto_id = ?`;

  db.run(sql, [nombre,codigo, precio,descripcion, sexo, empresa, categoria , id], err => {
  if (err) return console.error(err.message);
...
function mostrarDelete(req,res){
  ...
  db.get(sql, [id], (err, row) => {
    ...
    res.render('prod3.ejs', { modelo:row});
  });
}
...
function mostrarUpdateC(req,res){
    ...
    console.log(row);
    if (err) return console.error(err.message);
    res.render('cat2.ejs', {modelo:row});
  })
}
